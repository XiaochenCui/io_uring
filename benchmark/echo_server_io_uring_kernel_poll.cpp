...
// Initialize io_uring with kernel poll enabled.
//
// Return 0 for success.
int io_uring_init_with_kernel_poll(struct io_uring &ring)
{
...
int main(int argc, char *argv[])
{
    ...
    // initialize io_uring with kernel poll
    int init_result = io_uring_init_with_kernel_poll(ring);
    if (init_result != 0)
    {
        return init_result;
    }
